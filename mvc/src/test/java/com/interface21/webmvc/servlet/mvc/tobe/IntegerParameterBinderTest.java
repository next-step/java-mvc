...
class IntegerParameterBinderTest {

    private final IntegerParameterBinder binder = new IntegerParameterBinder();

    @DisplayName("support 매서드의 파라미터로 int 또는 Integer 타입의 Parameter 를 받으면 true 를 반환 한다")
    @Test
    public void support() throws Exception {
        ...
        when(parameter1.getType()).thenReturn((Class) int.class);
        when(parameter2.getType()).thenReturn((Class) Integer.class);
    ...
    @DisplayName("support 매서드의 파라미터로 int 또는 Integer 가 아닌 타입의 Parameter 를 받으면 false 를 반환 한다")
    @Test
